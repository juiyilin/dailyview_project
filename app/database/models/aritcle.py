...
def save_small_image_path(instance, filename):
    now = int(time.time())
    return f'article/{instance.uuid4_hex}/s-{now}-{filename}'


def save_main_image_path(instance, filename):
    now = int(time.time())
    return f'article/{instance.uuid4_hex}/m-{now}-{filename}'


def save_article_block_image_path(instance, filename):
    now = int(time.time())
...
class Article(models.Model):
    ...
    small_image = models.ImageField(upload_to=save_small_image_path, verbose_name='小圖')
    main_image = models.ImageField(upload_to=save_main_image_path, verbose_name='大圖')
...
class ArticleDetail(models.Model):
    ...
    image = models.ImageField(upload_to=save_article_block_image_path, null=True, verbose_name='圖')
